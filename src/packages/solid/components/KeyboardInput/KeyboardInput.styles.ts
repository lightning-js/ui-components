...
import type { KeyboardConfig, KeybpardStyles } from '../Keyboard/Keyboard.styles.js';
import type { KeyConfig, KeySizes, KeyStyles } from '../Key/Key.styles.js';

export interface KeyboardInputStyles {
  tone: Tone;
  Container: NodeStyleSet<{ keySpacing?: number; screenW?: number; marginX?: number }>;
  Keyboard: NodeStyleSet<{ baseWidth: number; sizes: KeySizes; contentColor: NodeStyles['color'] }>;
  Input: NodeStyleSet<{ keySpacing?: number; screenW?: number; marginX?: number }>;
}

export type KeyboardInputStyleProperties = {
  keySpacing?: number;
  screenW?: number;
  marginX?: number;
};

type KeyboardConfig = ComponentStyleConfig<KeyboardInputStyleProperties>;
...
const container: KeyboardInputConfig = {
  themeKeys: {
...
const Keyboard = makeComponentStyles<KeyboardStyles['Container']>(keyboard);
const Text = makeComponentStyles<KeyboardStyles['Text']>(text);

const styles: KeyboardStyles = {
  tone: defaultTone,
  Container,
  Keyboard, // TODO why was this wrapped in {}?
  Text
};
